...
//
// 		2022.03.1 Li YI
// 		Rmode to select different R
//		LBTmode to select LBT f.t
//
// ================================================================================
//
//		2021.08.24 Li YI
...
void compareRcp(int Rmode = 0, int LBTmode = 0, int drawData = 0) {
	...
	// Drawing
#ifdef DRAW
	unsigned int color[8] = {kMagenta+4, kBlue+4, kOrange+10, kPink+7, kGreen+4,kMagenta+4, kRed+4, kCyan};
	for(int t = 0; t<kTotal; t++) {
		for(int i = 0; i<Npt; i++) {
	...
	//if(drawData) l2->AddEntry(grData,Form("STAR Data %s",RLegName[Rmode]),"p");
